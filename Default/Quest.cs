...
namespace ConsoleApp8
{
    ...
    public class QuestBoard
    {
        ItemFactory item;
        Player player;
        public QuestBoard(ItemFactory item, Player player)
        {
            this.player = player;
            this.item = item;
            AddQuest();
        ...
        private void AddQuest()
        {
            QuestList.Add(new Quest
            {
                ...
                QuestReward = new List<Item> { item.Items[0]},
                QuestState = 0,
                QuestComplete = 5,
                Gold = 500
            });
            ...
            QuestList.Add(new Quest
            {
                ...
                QuestReward = new List<Item> { item.Items[4] },
                QuestState = 0,
                QuestComplete = 1,
                Gold = 300
            });
            ...
            QuestList.Add(new Quest
            {
                ...
                QuestRequest = "5레벨 달성",
                QuestReward = new List<Item> { item.Items[2], item.Items[5] },
            ...
            QuestList.Add(new Quest
            {
                ...
                "??????????????????????????????????????????????????????????????????????? \n" +
                "???????????????????????????????????????????????????????????????????????",
                QuestReward = new List<Item> { item.Items[6] },
                QuestState = 0,
                QuestComplete = 5,
                Gold = 999999999
            });
    ...
    public class QuestUI
    {
        QuestBoard board;
        ItemFactory item;
        public QuestUI(QuestBoard board, ItemFactory item)
        {
            this.board = board;
            this.item = item;
        }
